...
trips_per_line = defaultdict(
    dict
)  # e.g. '5': {'here': '1_8739366', 'there': '1_8739365'}
...
shared_brigades: dict[str, set[str]] = defaultdict(
    lambda: set()
)  # e.g. '4': {'17'} and '17': {'4'}
shared_brigades_count: dict[str, int] = defaultdict(
    lambda: int(0)
)  # there must be at least X shared trips to make lines share brigads
...
for _, trip in feed.trips.iterrows():  # type: Series
    ...
    if direction_name not in trips_per_line[route_id]:
        ...
        stop_ids = matching_stops[
            "stop_id"
        ].tolist()  # e.g. ['1706', '1703', '1705', ...
    ...
    if trip_number - 1 in trip_id_to_route_id:
        ...
        if other_line != route_id:
            logger.debug(
                f"Trip {trip_number} has two lines: {trip_id_to_route_id[trip_number - 1]} and {route_id}"
            )
            ...
            if shared_brigades_count[key] > 3:
                logger.info(
                    f"Lines {trip_id_to_route_id[trip_number - 1]} and {route_id} share brigades"
                )
...
for i, route in routes.iterrows():
    ...
    entry["przystankiSymbole"] = sorted(
        set(
    ...
    logger.info(
        f"Linia {route['route_id']}: {' - '.join(entry['petle'])} / {entry['przebieg']}"
    )
